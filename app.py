...
async def init_db():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute('''CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, nickname TEXT, rank TEXT DEFAULT 'Бронза', money INTEGER DEFAULT 0, bbc_money INTEGER DEFAULT 0, last_draw TEXT, titles TEXT DEFAULT 'Новичок', unlocked_titles TEXT DEFAULT 'Новичок')''')
        try: await db.execute("ALTER TABLE users ADD COLUMN bbc_money INTEGER DEFAULT 0")
        except: pass
        await db.execute('''CREATE TABLE IF NOT EXISTS cards (card_id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, rarity INTEGER, file_id TEXT)''')
        await db.execute('''CREATE TABLE IF NOT EXISTS inventory (user_id INTEGER, card_id INTEGER, count INTEGER DEFAULT 1, PRIMARY KEY (user_id, card_id))''')
        await db.execute('''CREATE TABLE IF NOT EXISTS promocodes (code TEXT PRIMARY KEY, reward_type TEXT, reward_val INTEGER, uses_left INTEGER, expires_at TEXT)''')
        await db.commit()
...
async def start_cmd(m: Message):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("INSERT OR IGNORE INTO users (user_id, nickname, rank, titles, unlocked_titles) VALUES (?,?,?,?,?)", (m.from_user.id, m.from_user.first_name, user_rank, title, title))
        await db.commit()
    await m.answer("Кнопки убраны. Используй команды или пиши словами!", reply_markup=ReplyKeyboardRemove())
    await profile_cmd(m)

@dp.message(Command("profile") | F.text.lower().in_(["профиль", "стата", "👤 профиль"]))
...
@dp.message(Command("draw") | F.text.lower().regexp(r"(^|\s)(карт[ау]|тянуть|хочу карту)($|\s)"))
async def draw_card(m: Message):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cd = timedelta(seconds=10) if rank == "Мифril" else timedelta(hours=24)
        ...
        if not card: return await m.answer(f"⚠️ Карт {rarity}⭐ нет в базе. Добавь их через /add_card")
...
@dp.message(Command("top") | F.text.lower().in_(["топ", "рейтинг", "🏆 топ"]))
async def top_menu(m: Message):
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="💰 Монеты", callback_data="top_money"), InlineKeyboardButton(text="💎 BBC", callback_data="top_bbc")]])
...
@dp.message(Command("inventory") | F.text.lower().in_(["инвентарь", "рюкзак", "🎒 инвентарь"]))
...
@dp.message(Command("help") | F.text.lower() == "помощь")
async def help_cmd(m: Message):
    await m.answer("Напиши 'карта', чтобы тянуть карточку, 'профиль' для инфы или 'инвентарь' для списка карт.")
...
async def admin_promo(m: Message):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            await db.commit()
        await m.answer(f"✅ Код {code} создан!")
    except: await m.answer("❌ Ошибка в формате.")
