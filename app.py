...
async def start_cmd(m: Message):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
    await m.answer("✅ Бот готов! Старые кнопки удалены. Пиши 'карта' или используй меню.", 
                   reply_markup=ReplyKeyboardRemove())

@dp.message(Command("profile") | F.text.lower().contains("профиль") | F.text.lower().contains("стата"))
...
async def draw_card(m: Message):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if not row:
            user_rank = "Мифрил" if m.from_user.id == ADMIN_ID else "Бронза"
            await db.execute("INSERT OR IGNORE INTO users (user_id, nickname, rank) VALUES (?,?,?)", (m.from_user.id, m.from_user.first_name, user_rank))
            await db.commit()
            res = await db.execute("SELECT rank, last_draw, titles FROM users WHERE user_id = ?", (m.from_user.id,))
            row = await res.fetchone()
...
@dp.message(Command("inventory") | F.text.lower().contains("инвент") | F.text.lower().contains("рюкзак"))
...
@dp.message(Command("help") | F.text.lower().contains("помощь"))
...
async def admin_add_card(m: Message):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            await db.commit()
        await m.answer(f"✅ Добавлена: {name} ({rarity}⭐)")
