...
rig_mode: str = "normal"  # "normal" or "win100"
rig_remaining: int = 0
...
# ── Глобальный перехватчик протухших коллбэков ──────────────────
@dp.errors()
async def global_error_handler(event, exception: Exception):
    """Молча подавляем TelegramBadRequest для старых callback query."""
    if isinstance(exception, TelegramBadRequest):
        if "query is too old" in str(exception):
            return True  # suppress — не логируем, не крашимся
    return False  # остальные ошибки пусть логируются как обычно
...
async def migrate_db():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cur = await db.execute(
            "SELECT value FROM game_settings WHERE key = 'rig_mode'"
        )
        row = await cur.fetchone()
        if row:
            rig_mode = row[0]
        cur2 = await db.execute(
            "SELECT value FROM game_settings WHERE key = 'rig_remaining'"
        )
        row2 = await cur2.fetchone()
        if row2:
            try:
                rig_remaining = int(row2[0])
            except ValueError:
                rig_remaining = 0
        await db.commit()
...
def consume_rig() -> bool:
    """If 100%% rig is active, consume one game and force a win."""
...
def generate_crash_point() -> float:
    cp = round(1.0 + random.expovariate(0.7), 2)
...
async def pay_cmd(m: Message):
    if not m.reply_to_message or not m.reply_to_message.from_user:
        return await m.answer(
            "💸 Ответь на сообщение игрока и напиши /pay [сумма]",
            reply_markup=back_menu_kb(),
        )
    args = (m.text or "").split()
    if len(args) < 2:
        return await m.answer(
            "💸 Формат: /pay [сумма] (в ответ на сообщение)",
            reply_markup=back_menu_kb(),
        )
    amount = parse_positive_int(args[1])
    if amount is None:
        return await m.answer("❌ Сумма должна быть целым числом > 0!")
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
    await m.answer(
        f"✅ Переведено {amount} 💰 игроку {recip[0]}!\n"
        f"(от {m.from_user.first_name} → {m.reply_to_message.from_user.first_name})"
    )
...
async def _start_crash(msg: Message, user_id: int, bet: int):
    if consume_rig():
        cp = round(random.uniform(8.0, 15.0), 2)
    else:
...
async def cards_list_cmd(m: Message):
    if m.from_user.id != ADMIN_ID:
        return
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT id, name, rarity FROM cards ORDER BY id")
        rows = await cur.fetchall()
    if not rows:
        return await m.answer("📋 Карт в базе пока нет.")
    lines = ["🃏 <b>СПИСОК КАРТ</b>\n"]
    for cid, name, rar in rows:
        lines.append(f"<code>#{cid}</code> | {'⭐' * rar} {RARITY_NAMES.get(rar, '?')} | {name}")
    text = "\n".join(lines)
    # Split into parts if too long for Telegram
    if len(text) <= 4000:
...
async def _save_rig_state():
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "INSERT OR REPLACE INTO game_settings (key, value) VALUES ('rig_mode', ?)",
            (rig_mode,),
        )
        await db.execute(
            "INSERT OR REPLACE INTO game_settings (key, value) VALUES ('rig_remaining', ?)",
            (str(rig_remaining),),
        )
        await db.commit()
...
async def rig_cmd(m: Message):
    ...
    if len(args) < 2:
        return await m.answer(
            "Формат:\n"
            "/rig 100 [кол-во игр] — 100%% побед\n"
            "/rig off — выключить подкрут"
        )
    if args[1].replace("%", "") == "100":
        count = parse_positive_int(args[2]) if len(args) >= 3 else None
        if count is None:
            return await m.answer("❌ Укажи кол-во игр: /rig 100 10")
        rig_mode = "win100"
        rig_remaining = count
        await _save_rig_state()
        await m.answer(f"🎯 Подкрут: 100%% побед на {count} игр!")
    elif args[1] in ("off", "normal", "0"):
        rig_mode = "normal"
        rig_remaining = 0
        await _save_rig_state()
        await m.answer("⚖️ Подкрут выключен.")
    else:
...
async def adm_rig_cb(c: CallbackQuery):
    if c.from_user.id != ADMIN_ID:
        return
    status = (
        f"🎯 100% побед — осталось <b>{rig_remaining}</b> игр"
        if rig_mode == "win100" else "⚖️ Выключен"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def adm_rig_on_cb(c: CallbackQuery, state: FSMContext):
    ...
    await safe_edit(
        c.message,
        "🎯 Введи кол-во игр с гарантированным выигрышем:",
        InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 Отмена", callback_data="adm_rig")]
        ]),
    )
    await c.answer()
...
async def adm_rig_count_handler(m: Message, state: FSMContext):
    if m.from_user.id != ADMIN_ID:
        return
    global rig_mode, rig_remaining
    ...
    if count is None:
        await state.clear()
        return await m.answer("❌ Введи целое число > 0!")
    rig_mode = "win100"
    rig_remaining = count
    await _save_rig_state()
    await state.clear()
    await m.answer(
        f"🎯 Подкрут включён!\n100% побед на <b>{count}</b> игр.",
        parse_mode=ParseMode.HTML,
    )
...
async def main():
    ...
    logging.info("Bot started! Rig: %s (%d games)", rig_mode, rig_remaining)
