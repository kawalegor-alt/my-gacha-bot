...
arena_queue: list = []  # [(user_id, message)]
...
async def cmd_arena(m: Message, state: FSMContext):
    ...
    if any(uid == m.from_user.id for uid, _ in arena_queue):
...
async def cmd_quests(m: Message):
    ...
    all_done = row[3] and row[7] and row[11]
    all_claimed = row[12]
...
async def cb_quest_claim(c: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        row = await cur.fetchone()
        if not row or not row[0]:
            return await c.answer("❌ Квест не выполнен!", show_alert=True)
        # Mark as 2 (claimed)
        await db.execute(
...
async def cmd_slots_bet(m: Message):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if mult > 0:
            await db.execute("UPDATE users SET balance=balance+? WHERE user_id=?", (bet * mult, m.from_user.id))
            result = f"🎉 <b>Выигрыш x{mult}! +{bet*mult:,} 💰</b>"
...
async def _resolve_roulette(m: Message, chat_id: int):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE users SET balance=balance-? WHERE user_id=?", (bet, loser["uid"]))
        for w in winners:
