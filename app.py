...
async def adm_pets_panel_cb(c: CallbackQuery):
    ...
    try:
        ...
        await safe_edit(c.message, f"🐾 <b>Питомцы</b>\n\n⚠️ Таблица не найдена.\nПерезапустите бота для создания таблиц.\n<code>{e}</code>", admin_pets_kb())
    await c.answer()
...
async def adm_arena_panel_cb(c: CallbackQuery):
    ...
    try:
        ...
        await safe_edit(c.message, f"⚔️ <b>Арена</b>\n\n⚠️ Таблица не найдена.\nПерезапустите бота для создания таблиц.\n<code>{e}</code>", admin_arena_kb())
    await c.answer()
...
async def adm_clans_panel_cb(c: CallbackQuery):
    ...
    try:
        ...
        await safe_edit(c.message, f"🏰 <b>Кланы</b>\n\n⚠️ Таблица не найдена.\nПерезапустите бота для создания таблиц.\n<code>{e}</code>", admin_clans_kb())
    await c.answer()
...
async def adm_bp_panel_cb(c: CallbackQuery):
    ...
    try:
        ...
        await safe_edit(c.message, f"🏆 <b>Боевой пропуск</b>\n\n⚠️ Таблица не найдена.\n<code>{e}</code>", admin_bp_kb())
    await c.answer()
...
async def adm_clan_list_cb(c: CallbackQuery):
    ...
    try:
        ...
        if not clans:
            text += "Кланов нет."
        await safe_edit(c.message, text, admin_clans_kb())
    except Exception as e:
        await safe_edit(c.message, f"📋 <b>Список кланов</b>\n\n⚠️ Ошибка: <code>{e}</code>", admin_clans_kb())
    await c.answer()
...
async def adm_clan_del_cb(c: CallbackQuery, state: FSMContext):
    if c.from_user.id != ADMIN_ID:
        return
    await state.set_state(AdminV5States.waiting_for_clan_del)
...
async def adm_del_pet_cb(c: CallbackQuery, state: FSMContext):
    if c.from_user.id != ADMIN_ID:
        return
    await state.set_state(AdminV5States.waiting_for_pet_del)
    await safe_edit(c.message, "🗑 Введи ID игрока для удаления питомца:")
    await c.answer()
...
async def adm_arena_reset_cb(c: CallbackQuery, state: FSMContext):
    if c.from_user.id != ADMIN_ID:
        return
    await state.set_state(AdminV5States.waiting_for_arena_reset)
    await safe_edit(c.message, "🔄 Введи ID игрока для сброса ЭЛО арены (до 1000):")
    await c.answer()
