...
class AdminV5States(StatesGroup):
    waiting_for_season_data = State()
    waiting_for_bp_give = State()
    waiting_for_clan_del = State()
    waiting_for_pet_del = State()
...
class AdminStates(StatesGroup):
    ...
    waiting_for_force_divorce_id = State()
    ...
    waiting_for_give_bpxp_data = State()
...
class BanCheckMiddleware(BaseMiddleware):
    ...
    async def __call__(self, handler, event, data):
        ...
        try:
            ...
            # DB error — let the request through rather than block everyone
            return await handler(event, data)
...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("""CREATE TABLE IF NOT EXISTS battlepass (
            user_id INTEGER,
        ...
        await db.execute("""CREATE TABLE IF NOT EXISTS bp_missions (
            user_id INTEGER,
            mission_date TEXT,
            m1_type TEXT, m1_target INTEGER DEFAULT 0, m1_progress INTEGER DEFAULT 0, m1_done INTEGER DEFAULT 0,
            m2_type TEXT, m2_target INTEGER DEFAULT 0, m2_progress INTEGER DEFAULT 0, m2_done INTEGER DEFAULT 0,
            m3_type TEXT, m3_target INTEGER DEFAULT 0, m3_progress INTEGER DEFAULT 0, m3_done INTEGER DEFAULT 0,
            PRIMARY KEY (user_id, mission_date)
        )""")
        await db.execute("""CREATE TABLE IF NOT EXISTS bp_weekly_missions (
            user_id INTEGER,
            week_start TEXT,
            m1_type TEXT, m1_target INTEGER DEFAULT 0, m1_progress INTEGER DEFAULT 0, m1_done INTEGER DEFAULT 0,
            m2_type TEXT, m2_target INTEGER DEFAULT 0, m2_progress INTEGER DEFAULT 0, m2_done INTEGER DEFAULT 0,
        ...
        await db.execute("""CREATE TABLE IF NOT EXISTS current_daily_event (
        ...
        await db.execute("""CREATE TABLE IF NOT EXISTS blackjack_games (
            user_id INTEGER PRIMARY KEY,
            bet INTEGER,
            player_hand TEXT,
            dealer_hand TEXT,
...
def main_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="⛏️ Шахта", callback_data="menu_mine"),
         InlineKeyboardButton(text="🔧 Крафт", callback_data="menu_craft")],
        [InlineKeyboardButton(text="🏰 Кланы", callback_data="menu_clan"),
         InlineKeyboardButton(text="🐾 Питомцы", callback_data="menu_pet")],
        [InlineKeyboardButton(text="⚔️ Арена", callback_data="menu_arena"),
         InlineKeyboardButton(text="🏆 Боевой пропуск", callback_data="menu_bp")],
        ...
        [InlineKeyboardButton(text="🃏 Карты", callback_data="menu_cards"),
         InlineKeyboardButton(text="👥 Общение", callback_data="menu_social")],
        [InlineKeyboardButton(text="🌟 Событие дня", callback_data="menu_event"),
...
# ── Меню → Питомцы ──────────────────────────────────────────
@dp.callback_query(F.data == "menu_pet")
async def menu_pet_cb(c: CallbackQuery, state: FSMContext):
    uid = c.from_user.id
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("INSERT OR IGNORE INTO users (user_id) VALUES (?)", (uid,))
        await db.execute("""CREATE TABLE IF NOT EXISTS pets (
            user_id INTEGER PRIMARY KEY, pet_type TEXT DEFAULT '', pet_name TEXT DEFAULT '',
            level INTEGER DEFAULT 1, hunger INTEGER DEFAULT 100, last_fed TEXT DEFAULT '',
            xp INTEGER DEFAULT 0)""")
        await db.commit()
        row = await get_pet(db, uid)
    if row:
        pet_info = PET_CATALOG.get(row[1], {})
        hunger_bar = "🟩" * (row[4] // 10) + "⬛" * (10 - row[4] // 10)
        lvl_xp_needed = row[3] * 100
        await safe_edit(
            c.message,
            f"🐾 <b>Твой питомец: {pet_info.get('name', row[1])}</b>\n"
            f"📛 Имя: <b>{row[2]}</b>\n"
            f"⭐ Уровень: <b>{row[3]}</b>\n"
            f"✨ XP: <b>{row[6]}/{lvl_xp_needed}</b>\n"
            f"🍽 Сытость: {hunger_bar} <b>{row[4]}%</b>\n\n"
            f"{'⚠️ Питомец голоден, бонусы не работают!' if row[4] <= 0 else '✅ Бонусы активны!'}\n\n"
            f"<i>Бонус монет: +{int(PET_CATALOG.get(row[1],{}).get('bonus_coins',0)*100*(1+(row[3]-1)*0.1))}%</i>\n"
            f"<i>Бонус XP: +{int(PET_CATALOG.get(row[1],{}).get('bonus_xp',0)*100*(1+(row[3]-1)*0.1))}%</i>",
            InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🍖 Покормить (500💰)", callback_data="pet_feed")],
                [InlineKeyboardButton(text="🔙 Меню", callback_data="menu_back")],
            ])
        )
    else:
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=f"{v['name']} — {v['price']:,}💰", callback_data=f"pet_buy:{k}")]
            for k, v in PET_CATALOG.items()
        ] + [[InlineKeyboardButton(text="🔙 Меню", callback_data="menu_back")]])
        await safe_edit(
            c.message,
            "🐾 <b>Магазин питомцев</b>\n\n"
            "Питомец даёт бонусы к монетам и XP.\n"
            "Кормите питомца, иначе бонусы пропадут!",
            kb
        )
    await c.answer()
...
async def menu_clan_cb(c: CallbackQuery, state: FSMContext):
    uid = c.from_user.id
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""CREATE TABLE IF NOT EXISTS clans (
            clan_id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE, tag TEXT,
            leader_id INTEGER, bank INTEGER DEFAULT 0, description TEXT DEFAULT '', created_at TEXT)""")
        await db.execute("""CREATE TABLE IF NOT EXISTS clan_members (
            user_id INTEGER PRIMARY KEY, clan_id INTEGER, role TEXT DEFAULT 'member')""")
        await db.commit()
        cur = await db.execute("SELECT clan_id, role FROM clan_members WHERE user_id=?", (uid,))
        member = await cur.fetchone()
        if member:
            clan_id, role = member
            cur2 = await db.execute("SELECT name, tag, bank, leader_id FROM clans WHERE clan_id=?", (clan_id,))
            clan = await cur2.fetchone()
            cur3 = await db.execute("SELECT COUNT(*) FROM clan_members WHERE clan_id=?", (clan_id,))
            cnt = (await cur3.fetchone())[0]
            name, tag, bank, leader = clan
            role_emoji = {"leader": "👑", "officer": "⭐", "member": "👤"}.get(role, "👤")
            kb = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="💰 Пополнить банк", callback_data="clan_deposit")],
                [InlineKeyboardButton(text="📋 Участники", callback_data=f"clan_members:{clan_id}")],
                [InlineKeyboardButton(text="⚔️ Рейд клана", callback_data="clan_raid")],
            ])
            if role == "leader":
                kb.inline_keyboard.insert(0, [
                    InlineKeyboardButton(text="✉️ Пригласить", callback_data="clan_invite"),
                    InlineKeyboardButton(text="🚪 Выгнать", callback_data="clan_kick"),
                ])
            ...
            kb.inline_keyboard.append([InlineKeyboardButton(text="🔙 Меню", callback_data="menu_back")])
            await safe_edit(
                c.message,
                f"🏰 <b>Клан [{tag}] {name}</b>\n\n"
                f"{role_emoji} Твоя роль: <b>{role}</b>\n"
                f"👥 Участников: <b>{cnt}</b>\n"
                f"💰 Банк клана: <b>{bank:,}</b>",
                kb
            )
        else:
            kb = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="➕ Создать клан (5000💰)", callback_data="clan_create")],
...
async def gacha_pull_cb(c: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if row[2]:
            return await c.answer("🥶 Аккаунт заморожен!", show_alert=True)
    ...
    if len(results) == 1 and results[0][3]:
        try:
            ...
            _track_msg(sent_photo.chat.id, sent_photo.message_id, uid)
        except Exception:
            await safe_edit(c.message, text, back_menu_kb())
    else:
        await safe_edit(c.message, text, back_menu_kb())
...
async def game_blackjack_cb(c: CallbackQuery, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cur = await db.execute("SELECT status FROM blackjack_games WHERE user_id=?", (uid,))
        row = await cur.fetchone()
        if row and row[0] == 'playing':
            await db.execute("DELETE FROM blackjack_games WHERE user_id=?", (uid,))
        await db.commit()
    await state.clear()
    await safe_edit(
        c.message,
...
def admin_main_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🐾 Питомцы", callback_data="adm_pets_panel"),
         InlineKeyboardButton(text="⚔️ Арена", callback_data="adm_arena_panel")],
        [InlineKeyboardButton(text="🏰 Кланы", callback_data="adm_clans_panel"),
         InlineKeyboardButton(text="🏆 Боевой пропуск", callback_data="adm_bp_panel")],
...
def admin_give_kb() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🏅 Дать ачивку", callback_data="adm_give_ach"),
         InlineKeyboardButton(text="🎁 Выдать уровень БП", callback_data="adm_bp_give")],
        [InlineKeyboardButton(text="✨ Выдать BP XP", callback_data="adm_give_bpxp")],
...
def admin_games_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="💔 Развести пару", callback_data="adm_force_divorce")],
...
def admin_pets_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🗑 Удалить питомца игрока", callback_data="adm_del_pet")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="adm_back")],
    ])
...
def admin_clans_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🗑 Удалить клан", callback_data="adm_clan_del")],
        [InlineKeyboardButton(text="📋 Список кланов", callback_data="adm_clan_list")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="adm_back")],
    ])


def admin_bp_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🎁 Выдать уровень БП", callback_data="adm_bp_give")],
        [InlineKeyboardButton(text="🔄 Новый сезон", callback_data="adm_bp_new_season")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="adm_back")],
    ])
...
async def word_trigger_card(m: Message):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await check_collection_achievements(db, uid)
        await update_bp_mission_progress(db, uid, "spin_gacha")
        await db.commit()
...
@dp.callback_query(F.data == "adm_force_divorce")
async def adm_force_divorce_cb(c: CallbackQuery, state: FSMContext):
    if c.from_user.id != ADMIN_ID:
        return
    await state.set_state(AdminStates.waiting_for_force_divorce_id)
    await safe_edit(c.message, "💔 Введи ID любого из супругов для принудительного развода:")
    await c.answer()


@dp.message(AdminStates.waiting_for_force_divorce_id)
async def process_force_divorce(m: Message, state: FSMContext):
    if m.from_user.id != ADMIN_ID:
        return
    uid = parse_positive_int(m.text)
    if not uid:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if not row or not row[0]:
            await state.clear()
            return await m.answer("❌ Игрок не в браке!")
        spouse = row[0]
        await db.execute(
            "UPDATE users SET spouse_id = 0, marriage_date = '' WHERE user_id IN (?, ?)",
            (uid, spouse),
        )
        await db.commit()
    await state.clear()
    await m.answer(
        f"💔 Брак между <code>{uid}</code> и <code>{spouse}</code> расторгнут.",
        parse_mode=ParseMode.HTML,
    )
...
MINE_CD = 1800  # 30 мин
FISH_CD = 600   # 10 мин
PET_HUNGER_RATE = 10  # единиц голода в час
CURRENT_BP_SEASON = 1
BP_LEVELS = 30

PET_CATALOG = {
    "cat":    {"name": "🐱 Кот",    "price": 1000,  "bonus_coins": 0.05, "bonus_xp": 0.05},
    "dog":    {"name": "🐶 Пёс",    "price": 2000,  "bonus_coins": 0.08, "bonus_xp": 0.03},
    "dragon": {"name": "🐉 Дракон", "price": 10000, "bonus_coins": 0.15, "bonus_xp": 0.10},
    "phoenix":{"name": "🔥 Феникс", "price": 25000, "bonus_coins": 0.20, "bonus_xp": 0.20},
}
...
QUEST_TYPES = [
    ...
    ("spin_gacha",   1,  400,   "🎴 Крути гачу {t} раз"),
    ("fish_catch",   3,  350,   "🎣 Поймай {t} рыбы"),
    ("win_duel",     1,  600,   "⚔️ Выиграй {t} дуэль"),
    ("mine_res",     2,  250,   "⛏ Добудь ресурсы {t} раза"),
]

BP_MISSIONS = [
    ("bp_play_game",     3,  150,  "🎮 Сыграй {t} раз в казино"),
    ("bp_earn_coins",    1000, 200, "💰 Заработай {t} монет"),
    ("bp_spin_gacha",    2,  100,  "🎴 Крути гачу {t} раз"),
    ("bp_catch_fish",    3,  120,  "🎣 Поймай {t} рыбы"),
    ("bp_win_duel",      1,  300,  "⚔️ Выиграй {t} дуэль"),
    ("bp_mine_resources", 2, 100,  "⛏ Добудь ресурсы {t} раза"),
    ("bp_daily_streak",  1,  80,   "📅 Собери ежедневку {t} раз"),
    ("bp_rob_someone",   1,  100,  "🦹 Ограбь {t} игрока"),
]

BP_WEEKLY_MISSIONS = [
    ("bpw_play_game",    15, 500,  "🎮 Сыграй {t} раз в казино"),
    ("bpw_earn_coins",   5000, 600, "💰 Заработай {t} монет"),
    ("bpw_win_duel",     5,  800,  "⚔️ Выиграй {t} дуэлей"),
    ("bpw_collect_cards", 10, 700, "🎴 Собери {t} карт"),
    ("bpw_mine_resources", 10, 400, "⛏ Добудь ресурсы {t} раз"),
]

DAILY_EVENTS = [
    ("double_work",    "💼 Двойная зарплата",    "Работа приносит x2 монет!"),
    ("bonus_xp",       "📈 Бонус XP",            "Все действия дают +50% XP!"),
    ("lucky_gacha",    "🍀 День удачи",          "Шанс на 4★+ карту в гаче повышен!"),
    ("free_wheel",     "🎡 Бесплатное колесо",   "Колесо удачи без кулдауна!"),
    ("mining_frenzy",  "⛏ Шахтёрский бум",      "Ресурсы из шахты x2!"),
    ("fishing_bonus",  "🎣 Рыбный день",         "Цена продажи рыбы x2!"),
    ("casino_luck",    "🎰 Фортуна",             "Шанс выигрыша в казино +10%!"),
]

CRAFT_RECIPES = {
    "luck_potion": {
        "name": "🍀 Зелье удачи",
        "desc": "Гарантирует 4★+ в следующей гаче",
        "cost": {"wood": 5, "stone": 3, "iron": 2},
        "effect": "lucky_gacha",
    },
    "shield_item": {
        "name": "🛡 Щит крафта",
        "desc": "Защита от следующего проигрыша в дуэли",
        "cost": {"wood": 10, "stone": 8, "iron": 5},
        "effect": "shield",
    },
    "coin_booster": {
        "name": "💎 Бустер монет",
        "desc": "x2 монеты на 1 час",
        "cost": {"wood": 8, "stone": 5, "iron": 10},
        "effect": "coin_mult",
    },
}
...
BP_REWARDS = {
    1:  ("coins", 200,   None),
    2:  ("xp",    500,   None),
    3:  ("coins", 500,   "premium_only"),
    5:  ("coins", 1000,  None),
    7:  ("bbc",   1,     "premium_only"),
    10: ("coins", 2000,  None),
    12: ("xp",    2000,  "premium_only"),
    15: ("bbc",   2,     None),
    18: ("coins", 3000,  "premium_only"),
    20: ("bbc",   3,     None),
    25: ("coins", 5000,  None),
    28: ("bbc",   5,     "premium_only"),
    30: ("bbc",   10,    None),
}
...
async def get_pet(db, user_id: int):
    cur = await db.execute("SELECT * FROM pets WHERE user_id = ?", (user_id,))
    return await cur.fetchone()


async def pet_bonus(db, user_id: int) -> tuple[float, float]:
    """Returns (coin_bonus_mult, xp_bonus_mult). 0.0 if no pet or hungry."""
    row = await get_pet(db, user_id)
    if not row:
        return 0.0, 0.0
    pet_type = row[1]
    hunger = row[4]
    if hunger <= 0:
        return 0.0, 0.0
    info = PET_CATALOG.get(pet_type, {})
    lvl_mult = 1.0 + (row[3] - 1) * 0.1  # +10% per level
    return info.get("bonus_coins", 0) * lvl_mult, info.get("bonus_xp", 0) * lvl_mult


async def ensure_resources(db, user_id: int):
    await db.execute(
        "INSERT OR IGNORE INTO resources (user_id) VALUES (?)", (user_id,)
    )
...
async def add_bp_xp(db, user_id: int, amount: int):
    """Grant BP XP and auto-level."""
    await db.execute(
        "INSERT OR IGNORE INTO battlepass (user_id, season) VALUES (?, ?)",
        (user_id, CURRENT_BP_SEASON)
    )
    cur = await db.execute(
        "SELECT level, bp_xp FROM battlepass WHERE user_id=? AND season=?",
        (user_id, CURRENT_BP_SEASON)
    )
    row = await cur.fetchone()
    if not row:
    ...
    lvl, xp = row
    xp += amount
    while xp >= lvl * 100 and lvl < BP_LEVELS:
        xp -= lvl * 100
        lvl += 1
    await db.execute(
        "UPDATE battlepass SET level=?, bp_xp=? WHERE user_id=? AND season=?",
        (lvl, xp, user_id, CURRENT_BP_SEASON)
    )
...
async def cmd_pet(m: Message, state: FSMContext):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("INSERT OR IGNORE INTO users (user_id) VALUES (?)", (m.from_user.id,))
        row = await get_pet(db, m.from_user.id)
    if row:
        pet_info = PET_CATALOG.get(row[1], {})
        hunger_bar = "🟩" * (row[4] // 10) + "⬛" * (10 - row[4] // 10)
        lvl_xp_needed = row[3] * 100
...
@dp.callback_query(F.data.startswith("pet_buy:"))
async def cb_pet_buy(c: CallbackQuery, state: FSMContext):
    pet_type = c.data.split(":")[1]
    info = PET_CATALOG.get(pet_type)
    if not info:
        return await c.answer("❌ Нет такого питомца")
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT balance FROM users WHERE user_id=?", (c.from_user.id,))
        row = await cur.fetchone()
        if not row or row[0] < info["price"]:
            return await c.answer(f"❌ Нужно {info['price']:,} монет!", show_alert=True)
        existing = await get_pet(db, c.from_user.id)
        if existing:
            return await c.answer("❌ У тебя уже есть питомец!", show_alert=True)
        await db.execute("UPDATE users SET balance=balance-? WHERE user_id=?", (info["price"], c.from_user.id))
        await db.execute(
            "INSERT INTO pets (user_id, pet_type, pet_name, hunger, last_fed) VALUES (?,?,?,100,?)",
            (c.from_user.id, pet_type, info["name"], datetime.now().isoformat())
        )
        await db.commit()
    await state.set_state(PetStates.waiting_for_pet_name)
    await state.update_data(pet_type=pet_type)
    await c.message.edit_text(
        f"✅ Куплен {info['name']}!\n\n✏️ Напиши имя для питомца:",
        parse_mode=ParseMode.HTML
    )
    await c.answer()


@dp.message(PetStates.waiting_for_pet_name)
async def process_pet_name(m: Message, state: FSMContext):
    name = (m.text or "").strip()[:20]
    if not name:
        return await m.answer("❌ Имя не может быть пустым!")
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE pets SET pet_name=? WHERE user_id=?", (name, m.from_user.id))
        await db.commit()
    await state.clear()
    await m.answer(f"🐾 Питомец назван <b>{name}</b>! Введи /pet чтобы посмотреть.", parse_mode=ParseMode.HTML)


@dp.callback_query(F.data == "pet_feed")
async def cb_pet_feed(c: CallbackQuery):
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT balance FROM users WHERE user_id=?", (c.from_user.id,))
        row = await cur.fetchone()
        if not row or row[0] < 500:
            return await c.answer("❌ Нужно 500 монет!", show_alert=True)
        pet = await get_pet(db, c.from_user.id)
        if not pet:
            return await c.answer("❌ У тебя нет питомца!")
        if pet[4] >= 100:
            return await c.answer("🍽 Питомец уже сыт!", show_alert=True)
        new_hunger = min(100, pet[4] + 50)
        await db.execute("UPDATE users SET balance=balance-500 WHERE user_id=?", (c.from_user.id,))
        await db.execute(
            "UPDATE pets SET hunger=?, last_fed=? WHERE user_id=?",
            (new_hunger, datetime.now().isoformat(), c.from_user.id)
        )
        await db.commit()
    await c.answer(f"🍖 Покормлено! Сытость: {new_hunger}%", show_alert=True)


# ── Фоновая задача: уменьшение голода питомцев ───────────────
async def pet_hunger_task():
    while True:
        await asyncio.sleep(3600)
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute("UPDATE pets SET hunger=MAX(0, hunger-10)")
            await db.commit()
...
async def _run_arena_battle(c: CallbackQuery, uid1: int, uid2: int, bet: int):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        # Balance update
        await db.execute("UPDATE users SET balance=balance-? WHERE user_id=?", (bet, uid1))
        await db.execute("UPDATE users SET balance=balance-? WHERE user_id=?", (bet, uid2))
...
async def cmd_fish(m: Message):
    async with aiosqlite.connect(DB_PATH) as db:
        await _ensure_game_tables(db)
        await db.execute("INSERT OR IGNORE INTO users (user_id) VALUES (?)", (m.from_user.id,))
        await db.execute("INSERT OR IGNORE INTO craft_items (user_id, item_type, count) VALUES (?, '_last_fish', 0)", (m.from_user.id,))
        await db.commit()
        cur = await db.execute(
        ...
        await db.execute(
            ...
            (m.from_user.id, fish_name)
        )
        # Update last_fish timestamp
        await db.execute(
            "UPDATE craft_items SET count=? WHERE user_id=? AND item_type='_last_fish'",
            (now_ts, m.from_user.id)
        )
        # Grant XP
        lvl_msg = await grant_xp(db, m.from_user.id, xp_reward)
        await add_bp_xp(db, m.from_user.id, 50)
        await update_quest_progress(db, m.from_user.id, "fish_catch")
        await update_bp_mission_progress(db, m.from_user.id, "catch_fish")
        await db.commit()
    ...
    await m.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💰 Продать весь улов", callback_data="fish_sell_all")],
...
async def cmd_auction(m: Message):
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "SELECT a.auction_id, a.card_name, a.card_rarity, a.current_bid, a.ends_at, a.seller_id, a.bidder_id "
            "FROM auctions a WHERE a.status='active' ORDER BY a.ends_at ASC LIMIT 10"
        )
        auctions = await cur.fetchall()

    if not auctions:
        text = "🏪 <b>Аукцион</b>\n\nАктивных лотов нет."
    else:
        text = "🏪 <b>Аукцион — Активные лоты</b>\n\n"
        for aid, cname, crarity, cbid, ends_at, seller, bidder in auctions:
            stars = RARITY_STARS.get(crarity, "⭐")
            try:
                ends = datetime.fromisoformat(ends_at)
                diff = ends - datetime.now()
                time_left = f"{int(diff.total_seconds()//60)}м" if diff.total_seconds() > 0 else "Завершён"
            except Exception:
                time_left = "?"
            text += f"#{aid} {stars} <b>{cname}</b>\n💰 Ставка: {cbid:,} | ⏰ {time_left}\n\n"

    kb_rows = [
        [InlineKeyboardButton(text="📤 Выставить карту", callback_data="auction_sell")],
    ]
    if auctions:
        kb_rows.append([InlineKeyboardButton(text="💰 Сделать ставку", callback_data="auction_bid_menu")])
    kb_rows.append([InlineKeyboardButton(text="🔙 Меню", callback_data="menu_back")])
    await m.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb_rows), parse_mode=ParseMode.HTML)


@dp.callback_query(F.data == "auction_sell")
async def cb_auction_sell(c: CallbackQuery, state: FSMContext):
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "SELECT uc.id, c.name, c.rarity FROM user_cards uc "
            "JOIN cards c ON uc.card_id=c.card_id WHERE uc.user_id=? LIMIT 20",
            (c.from_user.id,)
        )
        cards = await cur.fetchall()
    if not cards:
        return await c.answer("❌ У тебя нет карт!", show_alert=True)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text=f"{RARITY_STARS.get(r,'⭐')} {n}",
            callback_data=f"auction_pick:{uc_id}:{n[:20]}:{r}"
        )]
        for uc_id, n, r in cards
    ] + [[InlineKeyboardButton(text="🔙 Назад", callback_data="menu_back")]])
    await c.message.edit_text("📤 Выбери карту для аукциона:", reply_markup=kb)
    await c.answer()


@dp.callback_query(F.data.startswith("auction_pick:"))
async def cb_auction_pick(c: CallbackQuery, state: FSMContext):
    parts = c.data.split(":")
    uc_id, card_name, card_rarity = int(parts[1]), parts[2], int(parts[3])
    await state.set_state(AuctionStates.waiting_for_start_price)
    await state.update_data(uc_id=uc_id, card_name=card_name, card_rarity=card_rarity)
    await c.message.edit_text(
        f"💰 Карта: <b>{card_name}</b>\n\nВведи стартовую цену (минимум 100):",
        parse_mode=ParseMode.HTML
    )
    await c.answer()


@dp.message(AuctionStates.waiting_for_start_price)
async def process_auction_price(m: Message, state: FSMContext):
    price = parse_positive_int(m.text)
    if not price or price < 100:
        return await m.answer("❌ Минимальная цена: 100 монет!")
    data = await state.get_data()
    uc_id = data["uc_id"]
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        # Verify card still belongs to user
        cur = await db.execute("SELECT id FROM user_cards WHERE id=? AND user_id=?", (uc_id, m.from_user.id))
        if not await cur.fetchone():
            await state.clear()
            return await m.answer("❌ Карта не найдена!")
        # Check if card already on auction
        cur2 = await db.execute("SELECT auction_id FROM auctions WHERE uc_id=? AND status='active'", (uc_id,))
        if await cur2.fetchone():
            await state.clear()
            return await m.answer("❌ Карта уже на аукционе!")
        await db.execute(
            "INSERT INTO auctions (seller_id, uc_id, card_id, card_name, card_rarity, start_price, current_bid, ends_at) "
            "VALUES (?,?,?,?,?,?,?,?)",
            (m.from_user.id, uc_id, 0, card_name, card_rarity, price, price, ends_at)
        )
        await db.commit()
    await state.clear()
    await m.answer(
        f"✅ Карта <b>{card_name}</b> выставлена на аукцион!\n"
        f"💰 Стартовая цена: {price:,}\n⏰ Торги 1 час",
        parse_mode=ParseMode.HTML
    )


@dp.callback_query(F.data == "auction_bid_menu")
async def cb_auction_bid_menu(c: CallbackQuery, state: FSMContext):
    await state.set_state(AuctionStates.waiting_for_bid)
    await c.message.edit_text(
        "💰 Введи номер лота и ставку:\nФормат: <code>ID СТАВКА</code>\n\nПример: <code>1 5000</code>",
        parse_mode=ParseMode.HTML
    )
    await c.answer()
...
async def process_auction_bid(m: Message, state: FSMContext):
    parts = (m.text or "").split()
    if len(parts) < 2:
        await state.clear()
        return await m.answer("❌ Формат: ID СТАВКА")
    aid = parse_positive_int(parts[0])
    bid = parse_positive_int(parts[1])
    if not aid or not bid:
        await state.clear()
        return await m.answer("❌ Нужны числа!")
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "SELECT seller_id, current_bid, bidder_id, card_name, ends_at, status FROM auctions WHERE auction_id=?",
            (aid,)
        )
        row = await cur.fetchone()
        if not row or row[5] != 'active':
            await state.clear()
            return await m.answer("❌ Лот не найден или завершён!")
        seller_id, current_bid, prev_bidder, card_name, ends_at, _ = row
        if seller_id == m.from_user.id:
            await state.clear()
            return await m.answer("❌ Нельзя делать ставку на свой лот!")
        if bid <= current_bid:
            await state.clear()
            return await m.answer(f"❌ Ставка должна быть больше {current_bid:,}!")
        cur2 = await db.execute("SELECT balance FROM users WHERE user_id=?", (m.from_user.id,))
        bal = await cur2.fetchone()
        if not bal or bal[0] < bid:
            await state.clear()
            return await m.answer(f"❌ Нужно {bid:,} монет!")
        # Return coins to previous bidder
        if prev_bidder and prev_bidder != 0:
            await db.execute("UPDATE users SET balance=balance+? WHERE user_id=?", (current_bid, prev_bidder))
        # Deduct from new bidder
        await db.execute("UPDATE users SET balance=balance-? WHERE user_id=?", (bid, m.from_user.id))
        await db.execute(
            "UPDATE auctions SET current_bid=?, bidder_id=? WHERE auction_id=?",
            (bid, m.from_user.id, aid)
        )
        await db.commit()
    await state.clear()
    await m.answer(f"✅ Ставка {bid:,} 💰 на <b>{card_name}</b> принята!", parse_mode=ParseMode.HTML)


# Auction finalization task (runs every 5 minutes)
async def auction_finalize_task():
    while True:
        await asyncio.sleep(300)
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute("""CREATE TABLE IF NOT EXISTS auctions (
                auction_id INTEGER PRIMARY KEY AUTOINCREMENT,
                seller_id INTEGER, uc_id INTEGER, card_id INTEGER,
                card_name TEXT, card_rarity INTEGER,
                start_price INTEGER, current_bid INTEGER DEFAULT 0,
                bidder_id INTEGER DEFAULT 0, status TEXT DEFAULT 'active',
                created_at TEXT, ends_at TEXT
            )""")
            now = datetime.now().isoformat()
            cur = await db.execute(
                "SELECT auction_id, seller_id, uc_id, card_id, current_bid, bidder_id "
                "FROM auctions WHERE status='active' AND ends_at <= ?",
                (now,)
            )
            expired = await cur.fetchall()
            for aid, seller_id, uc_id, card_id, final_bid, bidder_id in expired:
                if bidder_id and bidder_id != 0:
                    # Transfer card to winner
                    await db.execute("UPDATE user_cards SET user_id=? WHERE id=?", (bidder_id, uc_id))
                    # Give coins to seller
                    await db.execute("UPDATE users SET balance=balance+? WHERE user_id=?", (final_bid, seller_id))
                    # Notify
                    try:
                        await bot.send_message(seller_id, f"🏪 Твоя карта с аукциона продана за {final_bid:,} 💰!")
                        await bot.send_message(bidder_id, f"🏪 Ты выиграл аукцион! Карта добавлена в коллекцию.")
                    except Exception:
                        pass
                else:
                    # No bids — return card to seller (it stays in their inventory, just remove from auction)
                    try:
                        await bot.send_message(seller_id, f"🏪 Лот на аукционе не продан — ставок не было.")
                    except Exception:
                        pass
                await db.execute("UPDATE auctions SET status='finished' WHERE auction_id=?", (aid,))
            await db.commit()
...
async def cmd_blackjack(m: Message, state: FSMContext):
    # Check existing game
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""CREATE TABLE IF NOT EXISTS blackjack_games (
            user_id INTEGER PRIMARY KEY,
            bet INTEGER, player_hand TEXT, dealer_hand TEXT,
            deck TEXT, status TEXT DEFAULT 'playing'
        )""")
        cur = await db.execute("SELECT status FROM blackjack_games WHERE user_id=?", (m.from_user.id,))
        row = await cur.fetchone()
        if row and row[0] == 'playing':
            await db.execute("DELETE FROM blackjack_games WHERE user_id=?", (m.from_user.id,))
            await db.commit()
    await m.answer(
        "🃏 <b>Блэкджек</b>\n\nВведи ставку (минимум 50):",
        parse_mode=ParseMode.HTML
    )
    await state.set_state(BlackjackStates.in_game)
...
async def process_blackjack_bet(m: Message, state: FSMContext):
    # Check if it's a bet or a game action
    data = await state.get_data()
    if "bet" not in data:
        # Expecting bet
        bet = parse_positive_int(m.text)
        if not bet or bet < 50:
            return await m.answer("❌ Минимальная ставка 50!")
        async with aiosqlite.connect(DB_PATH) as db:
            cur = await db.execute("SELECT balance FROM users WHERE user_id=?", (m.from_user.id,))
            row = await cur.fetchone()
            if not row or row[0] < bet:
        ...
        dealer_hand = [deck.pop(), deck.pop()]
        await state.update_data(bet=bet, deck=deck, player_hand=player_hand, dealer_hand=dealer_hand)
        await _show_blackjack(m, player_hand, dealer_hand, bet, state)
        return

    # Expecting action via inline keyboard - this path shouldn't normally be reached
...
async def cb_blackjack(c: CallbackQuery, state: FSMContext):
    ...
    if action == "bj_double":
        ...
        if ptotal_check > 21:
            # Bust on double — lose doubled bet
            async with aiosqlite.connect(DB_PATH) as db:
                await db.execute("UPDATE users SET balance=balance-? WHERE user_id=?", (bet, uid))
                await add_bp_xp(db, uid, 30)
                await db.commit()
            await state.clear()
            bj_end_kb = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔄 Ещё раз", callback_data="game_blackjack"),
...
async def cmd_slots(m: Message):
    args = (m.text or "").split()
    if len(args) < 2:
        return await m.answer("🎰 <b>Слоты</b>\n\nИспользование: <code>/slots 100</code>", parse_mode=ParseMode.HTML)
    m.text = " ".join(args[1:])  # pass args to the bet processor
    return await cmd_slots_bet(m)


async def cmd_slots_bet(m: Message):
    bet = parse_positive_int((m.text or "").split(maxsplit=1)[-1])
    if not bet or bet < 10:
        return await m.answer("❌ Минимальная ставка 10 монет! Пример: /slots 100")
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("INSERT OR IGNORE INTO users (user_id) VALUES (?)", (m.from_user.id,))
        cur = await db.execute("SELECT balance FROM users WHERE user_id=?", (m.from_user.id,))
        row = await cur.fetchone()
        ...
        winnings = bet * mult - (bet if mult == 0 else 0)
        await db.execute("UPDATE users SET balance=balance-? WHERE user_id=?", (bet, m.from_user.id))
        if mult > 0:
            winnings = bet * mult
            await db.execute("UPDATE users SET balance=balance+? WHERE user_id=?", (winnings, m.from_user.id))
            profit = winnings - bet
            result = f"🎉 <b>Выигрыш x{mult}! +{profit:,} 💰</b>"
            await update_quest_progress(db, m.from_user.id, "win_casino")
        else:
            result = f"💸 <b>Проигрыш! -{bet:,} 💰</b>"
        if mult == 100:
            result = f"🌟🌟🌟 <b>ДЖЕКПОТ! +{bet*100:,} 💰</b> 🌟🌟🌟"
        await add_bp_xp(db, m.from_user.id, 20)
        await db.commit()
...
async def _roulette_timeout(chat_id: int, seconds: int):
    ...
    if chat_id in roulette_sessions and roulette_sessions[chat_id]["status"] == "waiting":
        del roulette_sessions[chat_id]
        try:
            await bot.send_message(chat_id, "🔫 Рулетка отменена — никто не присоединился.")
        except Exception:
            pass
...
async def cmd_clan(m: Message, state: FSMContext):
    uid = m.from_user.id
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT clan_id, role FROM clan_members WHERE user_id=?", (uid,))
        member = await cur.fetchone()
...
async def cb_clan_create(c: CallbackQuery, state: FSMContext):
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT balance FROM users WHERE user_id=?", (c.from_user.id,))
        row = await cur.fetchone()
        if not row or row[0] < 5000:
            return await c.answer("❌ Нужно 5000 монет!", show_alert=True)
        cur2 = await db.execute("SELECT clan_id FROM clan_members WHERE user_id=?", (c.from_user.id,))
        if await cur2.fetchone():
            return await c.answer("❌ Ты уже в клане!", show_alert=True)
    await state.set_state(ClanStates.waiting_for_clan_name)
    await c.message.edit_text(
        "🏰 <b>Создание клана</b>\n\nВведи название клана (2-20 символов):",
        parse_mode=ParseMode.HTML
    )
    await c.answer()


@dp.message(ClanStates.waiting_for_clan_name)
async def process_clan_name(m: Message, state: FSMContext):
    name = (m.text or "").strip()
    if len(name) < 2 or len(name) > 20:
        return await m.answer("❌ Название: 2-20 символов!")
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT clan_id FROM clans WHERE name=?", (name,))
        if await cur.fetchone():
            return await m.answer("❌ Такой клан уже существует!")
    await state.update_data(clan_name=name)
    await state.set_state(ClanStates.waiting_for_clan_tag)
    await m.answer("🏷 Введи тег клана (2-5 букв, пример: BBС):")


@dp.message(ClanStates.waiting_for_clan_tag)
async def process_clan_tag(m: Message, state: FSMContext):
    tag = (m.text or "").strip().upper()[:5]
    if len(tag) < 2:
        return await m.answer("❌ Тег: 2-5 символов!")
    data = await state.get_data()
    name = data["clan_name"]
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT clan_id FROM clans WHERE name=?", (name,))
        if await cur.fetchone():
            await state.clear()
            return await m.answer("❌ Клан с таким именем уже существует!")
        await db.execute("UPDATE users SET balance=balance-5000 WHERE user_id=?", (m.from_user.id,))
        cur2 = await db.execute(
            "INSERT INTO clans (name, tag, leader_id, created_at) VALUES (?,?,?,?)",
            (name, tag, m.from_user.id, datetime.now().isoformat())
        )
        clan_id = cur2.lastrowid
        await db.execute(
            "INSERT INTO clan_members (user_id, clan_id, role) VALUES (?,?,?)",
            (m.from_user.id, clan_id, "leader")
        )
        await db.commit()
    await state.clear()
    await m.answer(
...
async def cb_clan_invite(c: CallbackQuery, state: FSMContext):
    await state.set_state(ClanStates.waiting_for_invite_id)
    await c.message.edit_text(
        "✉️ Введи ID пользователя для приглашения в клан:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 Назад", callback_data="menu_back")]
        ])
    )
...
@dp.message(ClanStates.waiting_for_invite_id)
async def process_clan_invite(m: Message, state: FSMContext):
    uid = parse_positive_int(m.text)
    if not uid:
        await state.clear()
        return await m.answer("❌ ID — число!")
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT clan_id FROM clan_members WHERE user_id=?", (inviter,))
        cm = await cur.fetchone()
        if not cm:
            await state.clear()
            return await m.answer("❌ Ты не в клане!")
        cur2 = await db.execute("SELECT clan_id FROM clan_members WHERE user_id=?", (uid,))
        if await cur2.fetchone():
            await state.clear()
            return await m.answer("❌ Игрок уже в клане!")
        cur3 = await db.execute("SELECT name, tag FROM clans WHERE clan_id=?", (cm[0],))
        clan = await cur3.fetchone()
        await db.execute(
            "INSERT INTO clan_members (user_id, clan_id, role) VALUES (?,?,?)",
            (uid, cm[0], "member")
        )
        await db.commit()
    await state.clear()
    await m.answer(f"✅ Игрок {uid} добавлен в клан [{clan[1]}] {clan[0]}!")
    try:
        await bot.send_message(uid, f"🏰 Тебя пригласили в клан <b>[{clan[1]}] {clan[0]}</b>!", parse_mode=ParseMode.HTML)
    except Exception:
        pass



@dp.callback_query(F.data == "clan_kick")
async def cb_clan_kick(c: CallbackQuery, state: FSMContext):
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT clan_id, role FROM clan_members WHERE user_id=?", (c.from_user.id,))
        row = await cur.fetchone()
        if not row or row[1] != "leader":
            return await c.answer("❌ Только лидер может выгнать!")
        clan_id = row[0]
        cur2 = await db.execute(
            "SELECT cm.user_id, u.nickname FROM clan_members cm "
            "LEFT JOIN users u ON cm.user_id=u.user_id "
            "WHERE cm.clan_id=? AND cm.user_id!=?",
            (clan_id, c.from_user.id)
        )
        members = await cur2.fetchall()
    if not members:
        return await c.answer("❌ Некого выгонять!", show_alert=True)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text=f"🚪 {nick or f'id{uid}'}",
            callback_data=f"clan_kick_confirm:{uid}"
        )]
        for uid, nick in members
    ] + [[InlineKeyboardButton(text="🔙 Назад", callback_data="menu_back")]])
    await c.message.edit_text("🚪 Выбери кого выгнать:", reply_markup=kb)
    await c.answer()


@dp.callback_query(F.data.startswith("clan_kick_confirm:"))
async def cb_clan_kick_confirm(c: CallbackQuery):
    uid = int(c.data.split(":")[1])
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT role FROM clan_members WHERE user_id=?", (c.from_user.id,))
        row = await cur.fetchone()
        if not row or row[0] != "leader":
            return await c.answer("❌ Нет прав!")
        await db.execute("DELETE FROM clan_members WHERE user_id=?", (uid,))
        await db.commit()
    await c.message.edit_text(f"✅ Игрок {uid} выгнан из клана.")
    await c.answer()

@dp.callback_query(F.data == "clan_search")
async def cb_clan_search(c: CallbackQuery):
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "SELECT c.clan_id, c.name, c.tag, COUNT(cm.user_id) as members "
            "FROM clans c LEFT JOIN clan_members cm ON c.clan_id=cm.clan_id "
            "GROUP BY c.clan_id ORDER BY members DESC LIMIT 10"
        )
        clans = await cur.fetchall()
    if not clans:
        return await c.answer("Нет кланов!", show_alert=True)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"[{tag}] {name} ({members}👥)", callback_data=f"clan_join:{cid}")]
        for cid, name, tag, members in clans
    ] + [[InlineKeyboardButton(text="🔙 Назад", callback_data="menu_back")]])
    await c.message.edit_text("🔍 <b>Кланы</b>\n\nВыбери для вступления:", reply_markup=kb, parse_mode=ParseMode.HTML)
    await c.answer()


@dp.callback_query(F.data.startswith("clan_join:"))
async def cb_clan_join(c: CallbackQuery):
    clan_id = int(c.data.split(":")[1])
    uid = c.from_user.id
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT clan_id FROM clan_members WHERE user_id=?", (uid,))
        if await cur.fetchone():
            return await c.answer("❌ Ты уже в клане!", show_alert=True)
        cur2 = await db.execute("SELECT name, tag FROM clans WHERE clan_id=?", (clan_id,))
        clan = await cur2.fetchone()
        if not clan:
            return await c.answer("❌ Клан не найден!")
        await db.execute(
            "INSERT INTO clan_members (user_id, clan_id, role) VALUES (?,?,?)",
            (uid, clan_id, "member")
        )
        await db.commit()
    await c.message.edit_text(
        f"✅ Вступил в клан <b>[{clan[1]}] {clan[0]}</b>!",
        parse_mode=ParseMode.HTML
    )
    await c.answer()
...
async def cb_clan_leave(c: CallbackQuery):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        row = await cur.fetchone()
        if not row:
            return await c.answer("❌ Ты не в клане!")
        if row[0] == "leader":
            return await c.answer("❌ Лидер не может покинуть клан! Сначала передай лидерство.", show_alert=True)
        await db.execute("DELETE FROM clan_members WHERE user_id=?", (c.from_user.id,))
        await db.commit()
    await c.message.edit_text("🚪 Ты покинул клан.")
    await c.answer()


@dp.callback_query(F.data == "clan_deposit")
async def cb_clan_deposit(c: CallbackQuery):
    await c.message.edit_text(
        "💰 Введи сумму для пополнения клан-банка:\n\nОтправь команду: <code>/clandeposit СУММА</code>",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 Назад", callback_data="menu_back")]
        ])
...
async def cmd_clan_deposit(m: Message):
    parts = (m.text or "").split()
    amount = parse_positive_int(parts[1]) if len(parts) > 1 else None
    if not amount:
        return await m.answer("❌ Использование: /clandeposit 1000")
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT balance FROM users WHERE user_id=?", (m.from_user.id,))
        bal = await cur.fetchone()
        if not bal or bal[0] < amount:
            return await m.answer("❌ Недостаточно монет!")
        cur2 = await db.execute("SELECT clan_id FROM clan_members WHERE user_id=?", (m.from_user.id,))
        cm = await cur2.fetchone()
        if not cm:
            return await m.answer("❌ Ты не в клане!")
        await db.execute("UPDATE users SET balance=balance-? WHERE user_id=?", (amount, m.from_user.id))
        await db.execute("UPDATE clans SET bank=bank+? WHERE clan_id=?", (amount, cm[0]))
        await db.commit()
    await m.answer(f"✅ Пополнено {amount:,} 💰 в клан-банк!")


@dp.callback_query(F.data == "clan_raid")
async def cb_clan_raid(c: CallbackQuery):
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT clan_id FROM clan_members WHERE user_id=?", (c.from_user.id,))
        cm = await cur.fetchone()
        if not cm:
            return await c.answer("❌ Ты не в клане!")
        clan_id = cm[0]
        cur2 = await db.execute("SELECT bank FROM clans WHERE clan_id=?", (clan_id,))
        clan = await cur2.fetchone()
        if not clan or clan[0] < 1000:
            return await c.answer("❌ В клан-банке нужно минимум 1000 монет для рейда!", show_alert=True)
        # Simple raid mechanic
        success = random.random() < 0.6
        loot = random.randint(500, 3000) if success else 0
        cost = 500  # Raid cost from clan bank
        await db.execute("UPDATE clans SET bank=bank-? WHERE clan_id=?", (cost, clan_id))
        if success:
            await db.execute("UPDATE users SET balance=balance+? WHERE user_id=?", (loot, c.from_user.id))
            await add_bp_xp(db, c.from_user.id, 100)
        await db.commit()
    if success:
        await c.message.edit_text(
            f"⚔️ <b>Рейд успешен!</b>\n\n"
            f"💰 Ты получил: +{loot:,} монет\n"
            f"🏰 Из клан-банка потрачено: {cost:,}",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔙 Назад", callback_data="menu_back")]
            ])
...
async def cb_clan_members(c: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "SELECT cm.user_id, cm.role, u.nickname FROM clan_members cm "
    ...
    for uid, role, nick in members:
        text += f"{role_emoji.get(role,'👤')} {nick or f'id{uid}'} — {role}\n"
    await c.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Назад", callback_data="menu_back")]
    ]), parse_mode=ParseMode.HTML)
    await c.answer()
...
async def cb_clan_disband(c: CallbackQuery):
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT clan_id, role FROM clan_members WHERE user_id=?", (c.from_user.id,))
        row = await cur.fetchone()
        if not row or row[1] != "leader":
            return await c.answer("❌ Только лидер может распустить клан!")
        clan_id = row[0]
        await db.execute("DELETE FROM clan_members WHERE clan_id=?", (clan_id,))
        await db.execute("DELETE FROM clans WHERE clan_id=?", (clan_id,))
        await db.commit()
    await c.message.edit_text("🏰 Клан распущен.")
    await c.answer()
...
async def cmd_mine(m: Message):
    async with aiosqlite.connect(DB_PATH) as db:
        await _ensure_game_tables(db)
        await ensure_resources(db, m.from_user.id)
        await db.commit()
        cur = await db.execute("SELECT last_mine FROM resources WHERE user_id=?", (m.from_user.id,))
        row = await cur.fetchone()
        last_mine_str = row[0] if row else ""
        now_ts = int(time.time())
        if last_mine_str:
            try:
                last_ts = int(datetime.fromisoformat(last_mine_str).timestamp())
                if now_ts - last_ts < MINE_CD:
                    remaining = MINE_CD - (now_ts - last_ts)
                    return await m.answer(
                        f"⛏ Шахта отдыхает!\n⏳ Подожди ещё <b>{remaining//60}м {remaining%60}с</b>",
                        parse_mode=ParseMode.HTML
                    )
            except Exception:
                pass
        # Mine resources
        wood = random.randint(2, 8)
        stone = random.randint(1, 6)
        iron = random.randint(0, 3)
        await db.execute(
            "UPDATE resources SET wood=wood+?, stone=stone+?, iron=iron+?, last_mine=? WHERE user_id=?",
            (wood, stone, iron, datetime.now().isoformat(), m.from_user.id)
        )
        await add_bp_xp(db, m.from_user.id, 30)
        await update_quest_progress(db, m.from_user.id, "mine_res")
        await update_bp_mission_progress(db, m.from_user.id, "mine_resources")
        cur2 = await db.execute("SELECT wood, stone, iron FROM resources WHERE user_id=?", (m.from_user.id,))
    ...
    await m.answer(
        ...
        f"⚙️ +{iron} железа\n\n"
        f"<b>Склад:</b>\n"
        f"🌲 Дерево: {res[0]} | 🪨 Камень: {res[1]} | ⚙️ Железо: {res[2]}",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def cmd_craft(m: Message):
    target = m
    uid = m.from_user.id
    async with aiosqlite.connect(DB_PATH) as db:
        await _ensure_game_tables(db)
        await ensure_resources(db, uid)
        await db.commit()
        cur = await db.execute("SELECT wood, stone, iron FROM resources WHERE user_id=?", (uid,))
        row = await cur.fetchone()
    wood, stone, iron = row if row else (0, 0, 0)
    text = (
        f"🧪 <b>Крафт</b>\n\n"
        f"📦 Ресурсы: 🌲{wood} 🪨{stone} ⚙️{iron}\n\n"
        f"<b>Рецепты:</b>\n"
    )
    kb_rows = []
    for key, recipe in CRAFT_RECIPES.items():
        cost = recipe["cost"]
        can_craft = wood >= cost["wood"] and stone >= cost["stone"] and iron >= cost["iron"]
        cost_str = f"🌲{cost['wood']} 🪨{cost['stone']} ⚙️{cost['iron']}"
        text += f"{'✅' if can_craft else '❌'} <b>{recipe['name']}</b>\n   {recipe['desc']}\n   Стоимость: {cost_str}\n\n"
        if can_craft:
            kb_rows.append([InlineKeyboardButton(text=f"🔨 {recipe['name']}", callback_data=f"craft:{key}")])
    kb_rows.append([InlineKeyboardButton(text="🔙 Меню", callback_data="menu_back")])
    kb = InlineKeyboardMarkup(inline_keyboard=kb_rows)
    await m.answer(text, reply_markup=kb, parse_mode=ParseMode.HTML)
...
async def cb_craft_menu(c: CallbackQuery):
    uid = c.from_user.id
    async with aiosqlite.connect(DB_PATH) as db:
        await _ensure_game_tables(db)
        await ensure_resources(db, uid)
...
async def cb_craft_item(c: CallbackQuery):
    key = c.data.split(":")[1]
    recipe = CRAFT_RECIPES.get(key)
    if not recipe:
        return await c.answer("❌ Рецепт не найден!")
    cost = recipe["cost"]
    uid = c.from_user.id
    async with aiosqlite.connect(DB_PATH) as db:
        await _ensure_game_tables(db)
        ...
        cur = await db.execute("SELECT wood, stone, iron FROM resources WHERE user_id=?", (uid,))
        row = await cur.fetchone()
        if not row or row[0] < cost["wood"] or row[1] < cost["stone"] or row[2] < cost["iron"]:
            return await c.answer("❌ Недостаточно ресурсов!", show_alert=True)
        await db.execute(
            "UPDATE resources SET wood=wood-?, stone=stone-?, iron=iron-? WHERE user_id=?",
            (cost["wood"], cost["stone"], cost["iron"], uid)
        )
        # Apply effect
        effect = recipe["effect"]
        if effect == "lucky_gacha":
            await db.execute("UPDATE users SET lucky_gacha=lucky_gacha+1 WHERE user_id=?", (uid,))
        elif effect == "shield":
            await db.execute("UPDATE users SET shield=shield+1 WHERE user_id=?", (uid,))
        elif effect == "coin_mult":
            boost_until = (datetime.now() + timedelta(hours=1)).isoformat()
            await db.execute(
                "UPDATE users SET coin_multiplier=2.0, vip_until=? WHERE user_id=?",
                (boost_until, uid)
            )
        await db.commit()
    await c.answer(f"✅ Скрафтено: {recipe['name']}!", show_alert=True)


# ══════════════════════════════════════════════════════════════
# ║  MENU BRIDGE HANDLERS (кнопки меню → фичи)                ║
# ══════════════════════════════════════════════════════════════
...
async def _ensure_game_tables(db):
    ...
    # Add missing columns to auctions if table was created by old schema
    for col, ctype, default in [("card_name", "TEXT", "''"), ("card_rarity", "INTEGER", "0"), ("status", "TEXT", "'active'"), ("created_at", "TEXT", "''")]:
        try:
            await db.execute(f"ALTER TABLE auctions ADD COLUMN {col} {ctype} DEFAULT {default}")
        except Exception:
            pass
...
async def menu_fish_cb(c: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await add_bp_xp(db, uid, 50)
        await update_quest_progress(db, uid, "fish_catch")
        await update_bp_mission_progress(db, uid, "catch_fish")
        await db.commit()
...
@dp.callback_query(F.data == "menu_mine")
async def menu_mine_cb(c: CallbackQuery):
    uid = c.from_user.id
    async with aiosqlite.connect(DB_PATH) as db:
        await _ensure_game_tables(db)
        await ensure_resources(db, uid)
        await db.commit()
        cur = await db.execute("SELECT last_mine FROM resources WHERE user_id=?", (uid,))
        row = await cur.fetchone()
        last_mine_str = row[0] if row else ""
        now_ts = int(time.time())
        if last_mine_str:
            try:
                ...
                if now_ts - last_ts < MINE_CD:
                    remaining = MINE_CD - (now_ts - last_ts)
                    cur2 = await db.execute("SELECT wood, stone, iron FROM resources WHERE user_id=?", (uid,))
                    res = await cur2.fetchone()
                    await safe_edit(
                        c.message,
                        f"⛏ <b>Шахта отдыхает!</b>\n⏳ Подожди ещё <b>{remaining//60}м {remaining%60}с</b>\n\n"
                        f"📦 Склад: 🌲{res[0]} 🪨{res[1]} ⚙️{res[2]}",
                        InlineKeyboardMarkup(inline_keyboard=[
                            [InlineKeyboardButton(text="🧪 Крафт", callback_data="menu_craft")],
                            [InlineKeyboardButton(text="🔙 Меню", callback_data="menu_back")],
                        ]),
        ...
        await db.execute(
            "UPDATE resources SET wood=wood+?, stone=stone+?, iron=iron+?, last_mine=? WHERE user_id=?",
            (wood, stone, iron, datetime.now().isoformat(), uid),
        )
        await add_bp_xp(db, uid, 30)
        await update_quest_progress(db, uid, "mine_res")
        await update_bp_mission_progress(db, uid, "mine_resources")
        cur3 = await db.execute("SELECT wood, stone, iron FROM resources WHERE user_id=?", (uid,))
        res = await cur3.fetchone()
        await db.commit()

    text = (
        f"⛏ <b>Добыча ресурсов</b>\n\n"
        f"🌲 +{wood} дерева\n🪨 +{stone} камня\n⚙️ +{iron} железа\n\n"
        f"<b>Склад:</b> 🌲{res[0]} 🪨{res[1]} ⚙️{res[2]}"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⛏ Ещё копать", callback_data="menu_mine")],
        [InlineKeyboardButton(text="🧪 Крафт", callback_data="menu_craft")],
        [InlineKeyboardButton(text="🔙 Меню", callback_data="menu_back")],
    ])
    await safe_edit(c.message, text, kb)
...
@dp.callback_query(F.data == "menu_craft")
async def menu_craft_cb(c: CallbackQuery):
    uid = c.from_user.id
    async with aiosqlite.connect(DB_PATH) as db:
        await _ensure_game_tables(db)
        await ensure_resources(db, uid)
    ...
    for key, recipe in CRAFT_RECIPES.items():
        cost = recipe["cost"]
        can = wood >= cost["wood"] and stone >= cost["stone"] and iron >= cost["iron"]
        cost_str = f"🌲{cost['wood']} 🪨{cost['stone']} ⚙️{cost['iron']}"
        text += f"{'✅' if can else '❌'} <b>{recipe['name']}</b>\n   {recipe['desc']}\n   Стоимость: {cost_str}\n\n"
        if can:
            kb_rows.append([InlineKeyboardButton(text=f"🔨 {recipe['name']}", callback_data=f"craft:{key}")])
    kb_rows.append([InlineKeyboardButton(text="🔙 Меню", callback_data="menu_back")])
    await safe_edit(c.message, text, InlineKeyboardMarkup(inline_keyboard=kb_rows))
    await c.answer()
...
async def menu_auction_cb(c: CallbackQuery):
    uid = c.from_user.id
    async with aiosqlite.connect(DB_PATH) as db:
        await _ensure_game_tables(db)
        cur = await db.execute(
            "SELECT a.auction_id, a.card_name, a.card_rarity, a.current_bid, a.ends_at, a.seller_id "
            "FROM auctions a WHERE a.status='active' ORDER BY a.ends_at ASC LIMIT 10"
        )
        auctions = await cur.fetchall()
    ...
    if not auctions:
        ...
        text = "🏪 <b>Аукцион — Активные лоты</b>\n\n"
        for aid, cname, crarity, cbid, ends_at, seller in auctions:
            stars = RARITY_STARS.get(crarity, "⭐")
            try:
                ends = datetime.fromisoformat(ends_at)
                diff = ends - datetime.now()
                tl = f"{int(diff.total_seconds()//60)}м" if diff.total_seconds() > 0 else "Завершён"
            except Exception:
                tl = "?"
            text += f"#{aid} {stars} <b>{cname}</b>\n💰 Ставка: {cbid:,} | ⏰ {tl}\n\n"
...
async def menu_social_cb(c: CallbackQuery):
    text = (
        "👥 <b>ОБЩЕНИЕ</b>\n\n"
        "Эти команды используются в ответ на сообщение другого игрока:\n\n"
        "⚔️ <code>/duel [ставка]</code> — Вызвать на дуэль\n"
        "💒 <code>/marry</code> — Предложить брак\n"
        "💔 <code>/divorce</code> — Развестись\n"
        "🎁 <code>/gift [сумма]</code> — Подарить монеты\n"
        "💸 <code>/pay [сумма]</code> — Перевести монеты\n"
        "🔫 <code>/rob</code> — Ограбить игрока\n"
        "👻 <code>/nev</code> — Стать невидимкой\n\n"
        "💡 <i>Ответь на сообщение нужного игрока и введи команду!</i>"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Меню", callback_data="menu_back")],
    ])
...
async def cmd_bp(m: Message):
    uid = m.from_user.id
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("INSERT OR IGNORE INTO users (user_id) VALUES (?)", (uid,))
        await db.execute(
            "INSERT OR IGNORE INTO battlepass (user_id, season) VALUES (?,?)",
            (uid, CURRENT_BP_SEASON)
        )
        await db.commit()
        cur = await db.execute(
            "SELECT level, bp_xp, is_premium, claimed_levels FROM battlepass WHERE user_id=? AND season=?",
            (uid, CURRENT_BP_SEASON)
        )
        row = await cur.fetchone()
    if not row:
        return await m.answer("❌ Ошибка загрузки БП")
    lvl, xp, is_premium, claimed_str = row
    claimed = set(map(int, claimed_str.split(","))) if claimed_str else set()
    xp_needed = lvl * 100

    text = (
        f"🏆 <b>Боевой пропуск — Сезон {CURRENT_BP_SEASON}</b>\n"
        f"{'💎 ПРЕМИУМ' if is_premium else '🆓 Бесплатно'}\n\n"
        f"📊 Уровень: <b>{lvl}/{BP_LEVELS}</b>\n"
        f"✨ XP: <b>{xp}/{xp_needed}</b>\n\n"
        f"<b>Ближайшие награды:</b>\n"
    )
    available_claims = []
    for bp_lvl in sorted(BP_REWARDS.keys()):
        if bp_lvl > lvl + 3:
            break
        rtype, rval, req = BP_REWARDS[bp_lvl]
        if req == "premium_only" and not is_premium:
            status = "🔒 Премиум"
        elif bp_lvl in claimed:
            status = "✅ Получено"
        elif bp_lvl <= lvl:
            status = "🎁 Доступно!"
            available_claims.append(bp_lvl)
        else:
            status = f"Уровень {bp_lvl}"
        reward_txt = f"{rval:,} {'монет' if rtype=='coins' else 'BBC' if rtype=='bbc' else 'XP'}"
        text += f"  Уровень {bp_lvl}: {reward_txt} — {status}\n"

    kb_rows = []
    if available_claims:
        kb_rows.append([InlineKeyboardButton(text=f"🎁 Забрать награды ({len(available_claims)})", callback_data=f"bp_claim")])
    if not is_premium:
        kb_rows.append([InlineKeyboardButton(text="💎 Купить Премиум (10 BBC)", callback_data="bp_buy_premium")])
    kb_rows.append([InlineKeyboardButton(text="🔙 Меню", callback_data="menu_back")])
    await m.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb_rows), parse_mode=ParseMode.HTML)


@dp.callback_query(F.data == "bp_claim")
async def cb_bp_claim(c: CallbackQuery):
    uid = c.from_user.id
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "SELECT level, is_premium, claimed_levels FROM battlepass WHERE user_id=? AND season=?",
            (uid, CURRENT_BP_SEASON)
        )
        row = await cur.fetchone()
        if not row:
            return await c.answer("❌ Нет данных БП!")
        lvl, is_premium, claimed_str = row
        claimed = set(map(int, claimed_str.split(","))) if claimed_str else set()
        total_coins = 0
        total_bbc = 0
        total_xp = 0
        newly_claimed = []
        for bp_lvl, (rtype, rval, req) in BP_REWARDS.items():
            if bp_lvl <= lvl and bp_lvl not in claimed:
                if req == "premium_only" and not is_premium:
                    continue
                if rtype == "coins":
                    total_coins += rval
                    ...
                    total_xp += rval
                newly_claimed.append(bp_lvl)
        if not newly_claimed:
            return await c.answer("Нечего забирать!", show_alert=True)
        claimed.update(newly_claimed)
        claimed_str = ",".join(map(str, sorted(claimed)))
        await db.execute(
            "UPDATE battlepass SET claimed_levels=? WHERE user_id=? AND season=?",
            (claimed_str, uid, CURRENT_BP_SEASON)
        )
        if total_coins:
            await db.execute("UPDATE users SET balance=balance+? WHERE user_id=?", (total_coins, uid))
        if total_bbc:
            await db.execute("UPDATE users SET bbc_balance=bbc_balance+? WHERE user_id=?", (total_bbc, uid))
        if total_xp:
            await grant_xp(db, uid, total_xp)
        await db.commit()
    parts = []
    if total_coins: parts.append(f"+{total_coins:,} 💰")
    if total_bbc: parts.append(f"+{total_bbc} BBC")
    if total_xp: parts.append(f"+{total_xp} XP")
    await c.answer(f"✅ Забрано: {' | '.join(parts)}", show_alert=True)


@dp.callback_query(F.data == "bp_buy_premium")
async def cb_bp_premium(c: CallbackQuery):
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT bbc_balance FROM users WHERE user_id=?", (c.from_user.id,))
        row = await cur.fetchone()
        if not row or row[0] < 10:
            return await c.answer("❌ Нужно 10 BBC!", show_alert=True)
        await db.execute("UPDATE users SET bbc_balance=bbc_balance-10 WHERE user_id=?", (c.from_user.id,))
        await db.execute(
            "UPDATE battlepass SET is_premium=1 WHERE user_id=? AND season=?",
            (c.from_user.id, CURRENT_BP_SEASON)
        )
        await db.commit()
    await c.answer("💎 Премиум активирован! Все награды теперь доступны.", show_alert=True)
...
@dp.callback_query(F.data == "adm_pets_panel")
async def adm_pets_panel_cb(c: CallbackQuery):
    if c.from_user.id != ADMIN_ID:
        return
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""CREATE TABLE IF NOT EXISTS pets (
                user_id INTEGER PRIMARY KEY, pet_type TEXT DEFAULT '', pet_name TEXT DEFAULT '',
                level INTEGER DEFAULT 1, hunger INTEGER DEFAULT 100, last_fed TEXT DEFAULT '', xp INTEGER DEFAULT 0)""")
        await db.commit()
        cur = await db.execute("SELECT COUNT(*) FROM pets")
        total = (await cur.fetchone())[0]
    await safe_edit(
        c.message,
        f"🐾 <b>Питомцы</b>\n\nВсего питомцев: <b>{total}</b>",
        admin_pets_kb()
    )
    await c.answer()
...
@dp.callback_query(F.data == "adm_clans_panel")
async def adm_clans_panel_cb(c: CallbackQuery):
    if c.from_user.id != ADMIN_ID:
        return
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""CREATE TABLE IF NOT EXISTS clans (
        ...
        await db.execute("""CREATE TABLE IF NOT EXISTS clan_members (
                user_id INTEGER PRIMARY KEY, clan_id INTEGER, role TEXT DEFAULT 'member')""")
        await db.commit()
        cur = await db.execute("SELECT COUNT(*) FROM clans")
        total = (await cur.fetchone())[0]
    await safe_edit(
        c.message,
        f"🏰 <b>Кланы</b>\n\nВсего кланов: <b>{total}</b>",
        admin_clans_kb()
    )
    await c.answer()


@dp.callback_query(F.data == "adm_bp_panel")
async def adm_bp_panel_cb(c: CallbackQuery):
    if c.from_user.id != ADMIN_ID:
        return
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""CREATE TABLE IF NOT EXISTS battlepass (
                user_id INTEGER, season INTEGER DEFAULT 1, level INTEGER DEFAULT 1,
                bp_xp INTEGER DEFAULT 0, is_premium INTEGER DEFAULT 0, claimed_levels TEXT DEFAULT '',
                PRIMARY KEY (user_id, season))""")
        await db.commit()
        cur = await db.execute("SELECT COUNT(*) FROM battlepass WHERE is_premium=1")
        premium = (await cur.fetchone())[0]
        cur2 = await db.execute("SELECT COUNT(*) FROM battlepass WHERE season=?", (CURRENT_BP_SEASON,))
        total = (await cur2.fetchone())[0]
    await safe_edit(
        c.message,
        f"🏆 <b>Боевой пропуск</b>\n\nСезон: <b>{CURRENT_BP_SEASON}</b>\n"
...
@dp.callback_query(F.data == "adm_clan_list")
async def adm_clan_list_cb(c: CallbackQuery):
    if c.from_user.id != ADMIN_ID:
        return
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""CREATE TABLE IF NOT EXISTS clans (
        ...
        await db.commit()
        cur = await db.execute(
            "SELECT c.name, c.tag, c.bank, COUNT(cm.user_id) "
            "FROM clans c LEFT JOIN clan_members cm ON c.clan_id=cm.clan_id "
            "GROUP BY c.clan_id ORDER BY c.bank DESC LIMIT 15"
        )
        clans = await cur.fetchall()
    text = "📋 <b>Список кланов</b>\n\n"
    for name, tag, bank, members in clans:
        text += f"[{tag}] {name} — 💰{bank:,} | 👥{members}\n"
    if not clans:
        text += "Кланов нет."
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Кланы", callback_data="adm_clans_panel")]
    ...
    await safe_edit(c.message, text, kb)
    await c.answer()
@dp.callback_query(F.data == "adm_clan_del")
async def adm_clan_del_cb(c: CallbackQuery, state: FSMContext):
    if c.from_user.id != ADMIN_ID:
        return
    await state.set_state(AdminStates.waiting_for_clan_del)
    await safe_edit(c.message, "🗑 Введи ID или название клана для удаления:")
    await c.answer()
@dp.message(AdminV5States.waiting_for_clan_del)
async def process_clan_del(m: Message, state: FSMContext):
    if m.from_user.id != ADMIN_ID:
        return
    query = (m.text or "").strip()
    async with aiosqlite.connect(DB_PATH) as db:
        if query.isdigit():
            cur = await db.execute("SELECT clan_id, name FROM clans WHERE clan_id=?", (int(query),))
        else:
            cur = await db.execute("SELECT clan_id, name FROM clans WHERE name=?", (query,))
        clan = await cur.fetchone()
        if not clan:
            await state.clear()
            return await m.answer("❌ Клан не найден!")
        await db.execute("DELETE FROM clan_members WHERE clan_id=?", (clan[0],))
        await db.execute("DELETE FROM clans WHERE clan_id=?", (clan[0],))
        await db.commit()
    await state.clear()
    await m.answer(f"✅ Клан <b>{clan[1]}</b> удалён.", parse_mode=ParseMode.HTML)


@dp.callback_query(F.data == "adm_del_pet")
async def adm_del_pet_cb(c: CallbackQuery, state: FSMContext):
    if c.from_user.id != ADMIN_ID:
        return
    await state.set_state(AdminStates.waiting_for_pet_del)
    await safe_edit(c.message, "🗑 Введи ID юзера для удаления питомца:")
    await c.answer()
@dp.message(AdminV5States.waiting_for_pet_del)
async def process_pet_del(m: Message, state: FSMContext):
    if m.from_user.id != ADMIN_ID:
        return
    uid = parse_positive_int(m.text)
    if not uid:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("DELETE FROM pets WHERE user_id=?", (uid,))
        await db.commit()
    await state.clear()
    await m.answer(f"✅ Питомец игрока {uid} удалён.")
...
@dp.callback_query(F.data == "adm_bp_give")
async def adm_bp_give_cb(c: CallbackQuery, state: FSMContext):
    if c.from_user.id != ADMIN_ID:
        return
    await state.set_state(AdminV5States.waiting_for_bp_give)
    await safe_edit(c.message, "🎁 Введи данные: <code>USER_ID УРОВНИ</code>\nПример: 123456 5")
    await c.answer()


@dp.message(AdminV5States.waiting_for_bp_give)
async def process_bp_give(m: Message, state: FSMContext):
    if m.from_user.id != ADMIN_ID:
        return
    parts = (m.text or "").split()
    if len(parts) < 2:
        await state.clear()
        return await m.answer("❌ Формат: USER_ID УРОВНИ")
    uid = parse_positive_int(parts[0])
    levels = parse_positive_int(parts[1])
    if not uid or not levels:
        await state.clear()
        return await m.answer("❌ Нужны числа!")
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
        ...
        await db.execute(
            "UPDATE battlepass SET level=MIN(level+?,?) WHERE user_id=? AND season=?",
            (levels, BP_LEVELS, uid, CURRENT_BP_SEASON)
        )
        await db.commit()
    await state.clear()
    await m.answer(f"✅ Игроку {uid} добавлено {levels} уровней БП.")


@dp.callback_query(F.data == "adm_bp_new_season")
async def adm_bp_new_season_cb(c: CallbackQuery):
    if c.from_user.id != ADMIN_ID:
        return
    await c.answer(
        f"⚠️ Для смены сезона измени CURRENT_BP_SEASON в коде.\nТекущий: {CURRENT_BP_SEASON}",
        show_alert=True
    )
...
@dp.callback_query(F.data == "adm_give_bpxp")
async def adm_give_bpxp_cb(c: CallbackQuery, state: FSMContext):
    if c.from_user.id != ADMIN_ID:
        return
    await state.set_state(AdminStates.waiting_for_give_bpxp_data)
    await safe_edit(
        c.message,
        "✨ <b>Выдать BP XP</b>\n\n"
        "Формат: <code>USER_ID КОЛИЧЕСТВО_XP</code>\n"
        "Пример: <code>123456 500</code>",
    )
    await c.answer()


@dp.message(AdminStates.waiting_for_give_bpxp_data)
async def process_give_bpxp(m: Message, state: FSMContext):
    if m.from_user.id != ADMIN_ID:
        return
    args = (m.text or "").split()
    if len(args) != 2:
        await state.clear()
        return await m.answer("❌ Формат: USER_ID КОЛИЧЕСТВО_XP")
    uid = parse_positive_int(args[0])
    xp_amount = parse_positive_int(args[1])
    if not uid or not xp_amount:
        await state.clear()
        return await m.answer("❌ Оба — числа > 0!")
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
        ...
        await add_bp_xp(db, uid, xp_amount)
        await db.commit()
    await state.clear()
    await m.answer(
        f"✨ Игроку <code>{uid}</code> выдано {xp_amount} BP XP.",
        parse_mode=ParseMode.HTML,
    )


# ══════════════════════════════════════════════════════════════
# ║  🏆 БОЕВОЙ ПРОПУСК В МЕНЮ (с миссиями)                    ║
# ══════════════════════════════════════════════════════════════
...
def _random_bp_missions(count: int = 3) -> list:
    """Генерация случайных BP миссий."""
    chosen = random.sample(BP_MISSIONS, min(count, len(BP_MISSIONS)))
    return [(m[0], m[1], m[2], m[3].format(t=m[1])) for m in chosen]


def _random_bp_weekly(count: int = 2) -> list:
    """Генерация случайных еженедельных BP миссий."""
    chosen = random.sample(BP_WEEKLY_MISSIONS, min(count, len(BP_WEEKLY_MISSIONS)))
    return [(m[0], m[1], m[2], m[3].format(t=m[1])) for m in chosen]


async def _ensure_bp_tables(db):
    """Создать BP таблицы если они не существуют."""
    await db.execute("""CREATE TABLE IF NOT EXISTS battlepass (
        user_id INTEGER, season INTEGER DEFAULT 1,
        level INTEGER DEFAULT 1, bp_xp INTEGER DEFAULT 0,
        is_premium INTEGER DEFAULT 0, claimed_levels TEXT DEFAULT '',
        PRIMARY KEY (user_id, season))""")
    await db.execute("""CREATE TABLE IF NOT EXISTS bp_missions (
        user_id INTEGER, mission_date TEXT,
        m1_type TEXT, m1_target INTEGER DEFAULT 0, m1_progress INTEGER DEFAULT 0, m1_done INTEGER DEFAULT 0,
        m2_type TEXT, m2_target INTEGER DEFAULT 0, m2_progress INTEGER DEFAULT 0, m2_done INTEGER DEFAULT 0,
        m3_type TEXT, m3_target INTEGER DEFAULT 0, m3_progress INTEGER DEFAULT 0, m3_done INTEGER DEFAULT 0,
        PRIMARY KEY (user_id, mission_date))""")
    await db.execute("""CREATE TABLE IF NOT EXISTS bp_weekly_missions (
        ...
        PRIMARY KEY (user_id, week_start))""")
    await db.execute("""CREATE TABLE IF NOT EXISTS current_daily_event (
        event_date TEXT PRIMARY KEY, event_type TEXT,
        event_name TEXT, event_desc TEXT)""")
    await db.commit()


async def _ensure_bp_missions(db, user_id: int):
    """Создать BP миссии на сегодня если их нет."""
    await _ensure_bp_tables(db)
    today = datetime.now().strftime("%Y-%m-%d")
    cur = await db.execute(
        "SELECT m1_type FROM bp_missions WHERE user_id=? AND mission_date=?",
        (user_id, today)
    )
    if not await cur.fetchone():
        missions = _random_bp_missions(3)
        await db.execute(
            "INSERT OR IGNORE INTO bp_missions (user_id, mission_date, "
            "m1_type, m1_target, m2_type, m2_target, m3_type, m3_target) "
            "VALUES (?,?,?,?,?,?,?,?)",
            (user_id, today,
             missions[0][0], missions[0][1],
             missions[1][0], missions[1][1],
             missions[2][0], missions[2][1])
        )

    week_start = _get_week_start()
    cur2 = await db.execute(
        "SELECT m1_type FROM bp_weekly_missions WHERE user_id=? AND week_start=?",
        (user_id, week_start)
    )
    if not await cur2.fetchone():
        weeklies = _random_bp_weekly(2)
        await db.execute(
            "INSERT OR IGNORE INTO bp_weekly_missions (user_id, week_start, "
            "m1_type, m1_target, m2_type, m2_target) "
            "VALUES (?,?,?,?,?,?)",
            (user_id, week_start,
             weeklies[0][0], weeklies[0][1],
             weeklies[1][0], weeklies[1][1])
        )
    await db.commit()


async def update_bp_mission_progress(db, user_id: int, mission_type: str, amount: int = 1):
    """Обновить прогресс BP миссий."""
    await _ensure_bp_tables(db)
    today = datetime.now().strftime("%Y-%m-%d")
    # Daily missions
    cur = await db.execute(
        "SELECT m1_type,m1_target,m1_progress,m1_done, "
        "m2_type,m2_target,m2_progress,m2_done, "
        "m3_type,m3_target,m3_progress,m3_done "
        "FROM bp_missions WHERE user_id=? AND mission_date=?",
        (user_id, today)
    )
    row = await cur.fetchone()
    if row:
        for i, prefix in enumerate(["m1", "m2", "m3"]):
            qtype = row[i*4]
            qtarget = row[i*4+1]
            qprog = row[i*4+2]
            qdone = row[i*4+3]
            # Match bp_ prefix mission types
            if qtype and qtype.replace("bp_", "") == mission_type.replace("bp_", "") and not qdone:
                new_prog = min(qprog + amount, qtarget)
                new_done = 1 if new_prog >= qtarget else 0
                await db.execute(
                    f"UPDATE bp_missions SET {prefix}_progress=?, {prefix}_done=? "
                    f"WHERE user_id=? AND mission_date=?",
                    (new_prog, new_done, user_id, today)
                )
                if new_done:
                    bp_xp_reward = next(
                        (m[2] for m in BP_MISSIONS if m[0] == qtype), 100
                    )
                    await add_bp_xp(db, user_id, bp_xp_reward)
    ...
    cur2 = await db.execute(
        "SELECT m1_type,m1_target,m1_progress,m1_done, "
        "m2_type,m2_target,m2_progress,m2_done "
        "FROM bp_weekly_missions WHERE user_id=? AND week_start=?",
        (user_id, week_start)
    )
    row2 = await cur2.fetchone()
    if row2:
        for i, prefix in enumerate(["m1", "m2"]):
            qtype = row2[i*4]
            qtarget = row2[i*4+1]
            qprog = row2[i*4+2]
            qdone = row2[i*4+3]
            if qtype and qtype.replace("bpw_", "") == mission_type.replace("bpw_", "").replace("bp_", "") and not qdone:
                new_prog = min(qprog + amount, qtarget)
                new_done = 1 if new_prog >= qtarget else 0
                await db.execute(
                    f"UPDATE bp_weekly_missions SET {prefix}_progress=?, {prefix}_done=? "
                    f"WHERE user_id=? AND week_start=?",
                    (new_prog, new_done, user_id, week_start)
                )
                if new_done:
                    bp_xp_reward = next(
                        (m[2] for m in BP_WEEKLY_MISSIONS if m[0] == qtype), 300
                    )
                    await add_bp_xp(db, user_id, bp_xp_reward)
    await db.commit()
...
async def menu_bp_cb(c: CallbackQuery):
    uid = c.from_user.id
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("INSERT OR IGNORE INTO users (user_id) VALUES (?)", (uid,))
        await db.execute(
        ...
        await db.commit()
        await _ensure_bp_missions(db, uid)
        ...
        if not bp_row:
            return await c.answer("❌ Ошибка загрузки БП", show_alert=True)
        lvl, xp, is_premium, claimed_str = bp_row
        claimed = set(map(int, claimed_str.split(","))) if claimed_str else set()
        xp_needed = lvl * 100
        ...
        weekly_row = await cur3.fetchone()

    # Build progress bar
    bar_len = 15
    filled = min(bar_len, int(xp / max(1, xp_needed) * bar_len))
    bar = "█" * filled + "░" * (bar_len - filled)
    pct = min(100, int(xp / max(1, xp_needed) * 100))
    ...
    text += "━━━━━━━━━━━━━━━━━━━━\n"
    text += "📋 <b>Миссии дня:</b>\n"
    bp_mission_descs = {m[0]: m[3] for m in BP_MISSIONS}
    if daily_row:
        for i in range(3):
            qtype = daily_row[i*4]
            qtarget = daily_row[i*4+1]
            qprog = daily_row[i*4+2]
            qdone = daily_row[i*4+3]
            if qtype:
                desc = bp_mission_descs.get(qtype, qtype).format(t=qtarget)
                xp_reward = next((m[2] for m in BP_MISSIONS if m[0] == qtype), 100)
                status = "✅" if qdone else f"{qprog}/{qtarget}"
                text += f"  {'✅' if qdone else '📌'} {desc} [{status}] +{xp_reward}XP\n"
    else:
        text += "  Нет миссий\n"

    # Weekly missions
    text += "\n📋 <b>Миссии недели:</b>\n"
    bpw_mission_descs = {m[0]: m[3] for m in BP_WEEKLY_MISSIONS}
    if weekly_row:
        for i in range(2):
            qtype = weekly_row[i*4]
            qtarget = weekly_row[i*4+1]
            qprog = weekly_row[i*4+2]
            qdone = weekly_row[i*4+3]
            if qtype:
                desc = bpw_mission_descs.get(qtype, qtype).format(t=qtarget)
                xp_reward = next((m[2] for m in BP_WEEKLY_MISSIONS if m[0] == qtype), 300)
                status = "✅" if qdone else f"{qprog}/{qtarget}"
                text += f"  {'✅' if qdone else '📌'} {desc} [{status}] +{xp_reward}XP\n"
    else:
        text += "  Нет миссий\n"
    ...
    for bp_lvl in sorted(BP_REWARDS.keys()):
        ...
        if req == "premium_only" and not is_premium:
            ...
            status = "⏳"
        text += f"  {status} Ур.{bp_lvl}: {reward_txt}{'  💎' if req == 'premium_only' else ''}\n"
        shown += 1

    # Build keyboard
    kb_rows = []
    available = any(
        bp_lvl <= lvl and bp_lvl not in claimed
        and (req != "premium_only" or is_premium)
        for bp_lvl, (rtype, rval, req) in BP_REWARDS.items()
    )
    if available:
        kb_rows.append([InlineKeyboardButton(text="🎁 Забрать награды", callback_data="bp_claim")])
    if not is_premium:
        kb_rows.append([InlineKeyboardButton(text="💎 Купить Премиум (10 BBC)", callback_data="bp_buy_premium")])
    kb_rows.append([InlineKeyboardButton(text="📜 Все награды", callback_data="bp_all_rewards")])
    kb_rows.append([InlineKeyboardButton(text="🔙 Меню", callback_data="menu_back")])
...
@dp.callback_query(F.data == "bp_all_rewards")
async def bp_all_rewards_cb(c: CallbackQuery):
    uid = c.from_user.id
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "SELECT level, is_premium, claimed_levels FROM battlepass WHERE user_id=? AND season=?",
    ...
    text = f"🏆 <b>Все награды Боевого Пропуска</b>\n\nТвой уровень: <b>{lvl}</b>\n\n"
    for bp_lvl in sorted(BP_REWARDS.keys()):
        rtype, rval, req = BP_REWARDS[bp_lvl]
        reward_txt = f"{rval:,} {'монет' if rtype=='coins' else 'BBC' if rtype=='bbc' else 'XP'}"
        if bp_lvl in claimed:
            status = "✅"
        elif bp_lvl <= lvl:
            status = "🎁"
        elif req == "premium_only" and not is_premium:
            status = "🔒💎"
        else:
            status = "⏳"
        marker = "▸" if bp_lvl == lvl else " "
        text += f"{marker} Ур.{bp_lvl}: {status} {reward_txt}{'  💎' if req == 'premium_only' else ''}\n"
...
async def _ensure_daily_event(db) -> tuple:
    """Получить или сгенерировать событие дня. Returns (type, name, desc)."""
    await _ensure_bp_tables(db)
    today = datetime.now().strftime("%Y-%m-%d")
    cur = await db.execute(
        "SELECT event_type, event_name, event_desc FROM current_daily_event WHERE event_date=?",
        (today,)
    )
    row = await cur.fetchone()
    if row:
    ...
    await db.execute(
        "INSERT OR REPLACE INTO current_daily_event (event_date, event_type, event_name, event_desc) "
        "VALUES (?,?,?,?)",
        (today, event[0], event[1], event[2])
    )
    await db.commit()
    return event[0], event[1], event[2]


async def get_daily_event_type() -> str:
    """Получить тип текущего события дня."""
    async with aiosqlite.connect(DB_PATH) as db:
        event_type, _, _ = await _ensure_daily_event(db)
    return event_type


@dp.callback_query(F.data == "menu_event")
async def menu_event_cb(c: CallbackQuery):
    async with aiosqlite.connect(DB_PATH) as db:
        event_type, event_name, event_desc = await _ensure_daily_event(db)

    event_bonuses = {
        "double_work":   "🔹 Работа (/work) приносит x2 монет\n🔹 Действует весь день!",
        "bonus_xp":      "🔹 Все действия дают +50% XP\n🔹 Качайся быстрее!",
        "lucky_gacha":   "🔹 Повышен шанс на редкие карты\n🔹 Удача на вашей стороне!",
        "free_wheel":    "🔹 Колесо удачи без кулдауна\n🔹 Крути сколько хочешь!",
        "mining_frenzy": "🔹 Ресурсы из шахты удвоены\n🔹 Крафти больше предметов!",
        "fishing_bonus": "🔹 Цена продажи рыбы x2\n🔹 Время рыбачить!",
        "casino_luck":   "🔹 Шанс выигрыша в казино +10%\n🔹 Фортуна улыбается!",
    }

    now = datetime.now()
    hours_left = 23 - now.hour
    mins_left = 59 - now.minute
    next_event = f"{hours_left}ч {mins_left}мин"
    ...
    text = (
        ...
        f"<i>{event_desc}</i>\n\n"
        f"<b>Бонусы:</b>\n"
        f"{event_bonuses.get(event_type, 'Бонусы активны!')}\n\n"
        f"⏰ Следующее событие через: <b>{next_event}</b>\n\n"
        f"💡 <i>Событие меняется каждый день автоматически.\n"
        f"Все игроки получают одинаковые бонусы!</i>"
    )
...
async def main():
    ...
    asyncio.create_task(pet_hunger_task())
    asyncio.create_task(auction_finalize_task())
