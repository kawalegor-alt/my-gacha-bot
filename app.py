...
async def duel_fire_cb(c: CallbackQuery):
    ...
    if loser:
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            # WIPE loser completely (marriages preserved!)
            ...
            # NOTE: marriages NOT deleted — брак сохраняется после дуэли
...
async def cmd_roulette(m: Message):
    ...
    if chat_id in roulette_sessions and roulette_sessions[chat_id]["status"] == "waiting":
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            await db.execute("UPDATE users SET balance=balance-? WHERE user_id=? AND balance>=?", (bet, uid, bet))
            await db.commit()
