...
async def set_commands(bot: Bot):
    cmds = [
        BotCommand(command="start", description="🏠 Старт и очистка кнопок"),
...
async def start_cmd(m: Message):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
    # Удаление старых кнопок принудительно
    await m.answer("✅ Бот готов! Старые кнопки удалены. Используй меню команд или пиши словами.", 
...
@dp.message(Command("draw") | F.text.lower().regexp(r"(^|\s)(карт[ау]|тянуть|хочу карту|🃏 получить карту)($|\s)"))
async def draw_card(m: Message):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if not card: return await m.answer(f"⚠️ В базе нет карт {rarity}⭐. Добавь их через /add_card")
...
@dp.message(Command("inventory") | F.text.lower().in_(["инвентарь", "карты", "🎒 инвентарь"]))
async def inv_cmd(m: Message):
    ...
    if not cards: return await m.answer("У тебя пока нет карт!")
    text = "🎒 ТВОИ КАРТЫ:\n" + "\n".join([f"{r}⭐ {n} x{c}" for n,r,c in cards])
    await m.answer(text)

@dp.message(Command("top") | F.text.lower().in_(["топ", "🏆 топ"]))
...
async def help_cmd(m: Message):
    await m.answer("Напиши 'карта' чтобы тянуть, 'профиль' для статистики или 'инвентарь' для списка твоих карт.")
...
async def use_promo(m: Message):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
        await m.answer(f"✅ Успешно активировано!")
...
async def admin_help_cmd(m: Message):
    ...
    text = (
        "🛠 <b>АДМИН-ПАНЕЛЬ</b>\n\n"
        "/id_cards — Список всех ID карт\n"
        "/create_promo — Создать код\n"
        "📸 <b>Добавить карту:</b> Отправь фото с подписью:\n"
        "<code>/add_card Имя Редкость</code>"
    )
...
async def list_ids(m: Message):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cards = await res.fetchall()
    if not cards: return await m.answer("База пуста.")
...
async def admin_add_card(m: Message):
    ...
    except: await m.answer("❌ Ошибка. Пример: /add_card Имя 5")
...
async def admin_promo(m: Message):
    ...
    except: await m.answer("❌ Ошибка. Пример: /create_promo GIFT cash 100 5 7")
